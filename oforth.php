...
$rootDict->addWord('exec', function(Stack_ $stack, StringBuffer $buffer, string $word) use ($rootDict, $db) {
    $a = $stack->pop();
    if (empty($a)) {
        return;
    }
...
// todo translate to add:
$rootDict->addWord('+', function(Stack_ $stack, StringBuffer $buffer, string $word) use ($rootDict, $db) {
    // nothing
});
